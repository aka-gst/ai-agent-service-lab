...
Настоящий токен WB хранится только в локальном `.env` как `WB_API_TOKEN` и
никогда не добавляется в Git. До получения разрешения продавца интеграция
работает без токена и продолжает использовать демонстрационные CSV.
...
Пройдены лабораторные 1–10: Ollama API, structured output, безопасные инструменты, память SQLite, RAG, evaluation, FastAPI/Docker, передача клиенту, помощник аналитика маркетплейса и навигатор по интерфейсу. Проверяемый набор содержит 76 автоматических тестов.
